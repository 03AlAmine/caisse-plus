...
export class RapportService {
  ...
  async generer(dateDebut: Date, dateFin: Date, caisseId?: string): Promise<RapportData> {
  ...
  exportCSV(data: RapportData): void {
    const rows = [
      ['N° Pièce', 'Date', 'Libellé', 'Caisse', 'Catégorie', 'Type', 'Montant (FCFA)', 'Responsable'],
  ...
  async exportExcel(data: RapportData, caisseName?: string): Promise<void> {
    ...
    if (data.parCaisse.length > 1) {
      const caisseRows = [
        ...
        ...data.parCaisse.map(c => [c.nom, c.entrees, c.sorties, c.entrees - c.sorties]),
      ];
  ...
  async exportPDF(data: RapportData, caisseName?: string, orgName?: string): Promise<void> {
    ...
    const caisseTxt  = caisseName ? `Caisse : ${caisseName}` : 'Toutes les caisses';
    ...
    const pills  = [
      ...
      { label: 'Solde net',        val: data.soldeNet,     color: data.soldeNet >= 0 ? vert : rouge },
      { label: 'Nb opérations',    val: data.operations.length, color: bleuMarine, isMontant: false },
    ];
    ...
    pills.forEach((p, i) => {
      ...
      const valTxt = (p as any).isMontant === false
        ? String(p.val)
    ...
    autoTable(doc, {
      startY: yBande + 18,
      head: [['N° Pièce', 'Date', 'Désignation', 'Caisse', 'Catégorie', 'Entrées (FCFA)', 'Sorties (FCFA)', 'Responsable']],
      body: tableBody,
      styles: {
        fontSize: 7.5,
        cellPadding: 2,
      ...
      columnStyles: {
        ...
        3: { cellWidth: 28 },
        4: { cellWidth: 28 },
        5: { cellWidth: 28, halign: 'right', textColor: vert as [number,number,number] },
        6: { cellWidth: 28, halign: 'right', textColor: rouge as [number,number,number] },
        7: { cellWidth: 30 },
      },
  ...
  private _buildLivreRows(data: RapportData, caisseName?: string): any[][] {
    ...
    // En-têtes colonnes
    rows.push(['N° Pièce', 'Date', 'Désignation', 'Catégorie', 'Entrées (FCFA)', 'Sorties (FCFA)', 'Solde (FCFA)', 'Responsable']);

    // Ligne solde d'ouverture
    rows.push(['—', data.dateDebut.toLocaleDateString('fr-FR'), 'Report à nouveau / Solde d\'ouverture', '', '', '', '', '']);
